...
if __name__ == "__main__":
    ...
    Net = Model(opts)
    #FlowNet = networks.LiteFlowNet.LightFlowNet()
    ...
    #FlowNet.load_state_dict(remove_state_module(state_dict['flow_model']))
    ...
    for v in range(len(video_list)//2):
        ...
        output_dir_spa = os.path.join(opts.output_dir, "spa", opts.model_name, "epoch_%d" % opts.epoch, video)
        print(input_dir)
        print(output_dir)
        ...
        if not os.path.isdir(output_dir_spa):
            os.makedirs(output_dir_spa)
        ...
        for t in range(3, len(frame_list)-3):
            ...
            with torch.no_grad():
                ...
                time1 = time.time()
                frame_pred, frame_pred_spa, _, _ = Net(frame_input)
                time2 = time.time()
                print(time2 - time1)
            ...
            output_filename = os.path.join(output_dir, os.path.basename(frame_list[t]).replace(opts.file_suffix, '.png'))
            output_filename_spa = os.path.join(output_dir_spa, os.path.basename(frame_list[t]).replace(opts.file_suffix, '.png'))            
