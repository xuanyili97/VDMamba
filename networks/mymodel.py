...
class ResBlock(nn.Module):
    def __init__(self, din, n_block=1):
        ...
        for i in range(self.n_block):
            self.res_convs.append(
                nn.Sequential(
                    nn.Conv3d(din, din, (3, 3, 3), 1, (0,1,1)),
                    nn.PReLU(),
                    nn.Conv3d(din, din, (3, 3, 3), 1, (0,1,1)),
                    nn.PReLU()
                )
    ...
    def forward(self, x):
        for i in range(self.n_block):
            resx = x
            x = self.prelus[i](self.res_convs[i](x))
        return x
...
class MyPixelUnShuffle(nn.Module):
    def __init__(self, factor=2):
        super(MyPixelUnShuffle, self).__init__()
        if isinstance(factor, int):
            self.factor = [factor, factor]
        elif isinstance(factor, Tuple):
            self.factor = factor
...
class Model(nn.Module):
    def __init__(self, 
        ...
        self.reduce_chan_level_latent = ConvLSTM(int(dim*2**2))
        ...
        self.out = nn.Conv3d(out_channels, out_channels, (3,3,3), 1, (0,1,1))
...
#                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, inputs):
        ...
        center_frame = inputs[:,:,sq//2:sq//2+1]
        outs = []

        for i in range(sq):
#        for i in range(1, sq):
            frame = inputs[:,:,i]
            out_single = self.forward_single(frame)
            outs.append(out_single.unsqueeze(2))
        outs = torch.cat(outs, dim=2)
        out = self.post_iterative_enhance(outs)
        out = self.out(out)
        return out + center_frame
