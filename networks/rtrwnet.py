...
class Model(nn.Module):
    def __init__(self, opts, scales=[1]):
        ...
        self.frame_restorer = Restormer(inp_channels=39, out_channels=3, num_blocks=[1,1,1,1], num_refinement_blocks=2)
        if self.ifAlign:
            self.flow_estimator = Restormer(inp_channels=21, out_channels=12, num_blocks=[1,1,1,1], num_refinement_blocks=2)
#            self.flow_estimator = LightFlowNet()
            if self.ifAggregate:
                self.frame_restorer = Restormer(inp_channels=39, out_channels=23, num_blocks=[1,1,1,1], num_refinement_blocks=2)
        else:
